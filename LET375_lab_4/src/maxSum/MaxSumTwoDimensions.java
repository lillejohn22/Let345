...
public class MaxSumTwoDimensions {
    ...
    // O(n^6)
    public static int maxSubMatrixSumBad( int[][] a ) { // COMPLETE
        int maxSum = 0;

        for( int r1 = 0; r1 < a.length; r1++ ) // start row
            for( int c1 = 0; c1 < a.length; c1++ ) // start column
            	for( int r2 = r1; r2 < a.length; r2++ ) // end row
            		for( int c2 = c1; c2 < a.length; c2++ ) { // end column
            			
            			int columnSum = 0;
            			int thisSum = 0;
            			
            			for( int c3 = c1; c3 <= c2; c3++ ) { // c3 goes from c1 to c2 
            				for( int r3 = r1; r3 <= r2; r3++ ) //  r3 goes from r1 to r2
            					columnSum += a[r3][c3];
            				thisSum += columnSum;
            				columnSum = 0;
            			}
        ...
        return Math.max(0, maxSum); // If sum is negative, set maxSum to sum of 0x0 matrix
    }
 
    // O(n^5)
    public static int maxSubMatrixSumBetter( int[][] a ) { // COMPLETE
        int maxSum = 0;
    ...
    // O(n^4)
    public static int maxSubMatrixSumEvenBetter( int[][] a ) { // COMPLETE
    	...
    	for( int r1 = 0; r1 < a.length; r1++ )
    		for( int r2 = r1; r2 < a.length; r2++) {
    			for( int c1 = 0; c1 < a.length; c1++ ) {
    				...
    				thisSum = Math.max(0, thisSum + columnSum);  
    				maxSum = Math.max(thisSum, maxSum);
    			}
    			thisSum = 0;
    		}
...
// Uncomment as you proceed!
    ...
    public static void main(String[] arg) {
        ...
        int[][] sampleMatrix = {     // max sum is 21
         ...
         // Our own test matrix, all negative elements
         int[][] negativeMatrix = {
        		 {-1,-2,-6},
        		 {-7,-9,-2}
         };
